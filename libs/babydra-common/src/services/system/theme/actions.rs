...
pub fn apply_appearance(gtk_theme: &str, icon_theme: &str, cursor_theme: &str, cursor_size: u32) -> Result<(), String> {
    let size_str = cursor_size.to_string();
    
    // Apply via gsettings
    let _ = run_cmd(&["gsettings", "set", "org.gnome.desktop.interface", "gtk-theme", gtk_theme]);
    let _ = run_cmd(&["gsettings", "set", "org.gnome.desktop.interface", "icon-theme", icon_theme]);
    let _ = run_cmd(&["gsettings", "set", "org.gnome.desktop.interface", "cursor-theme", cursor_theme]);
    let _ = run_cmd(&["gsettings", "set", "org.gnome.desktop.interface", "cursor-size", &size_str]);
            ...
            for line in content.lines() {
                ...
                if let Some((key, _)) = trimmed.split_once('=') {
                    match key {
                        ...
                        _ => {
                            env_vars.push((key.to_string(), trimmed.split_once('=').unwrap().1.to_string()));
                        }
                    }
                } else {
                    env_vars.push((line.to_string(), String::new()));

...
pub fn wire_rubberband(
    ...
    type RowRect = (ListBoxRow, f64, f64, f64, f64);
    ...
    drag_gesture.connect_drag_begin(move |gesture, x, y| {
        ...
        if is_item {
            ...
            // Snapshot row positions once for the whole rubberband gesture
            let mut rects = Vec::new();
            let mut child = lb_begin.first_child();
            while let Some(c) = child {
                if let Some(row) = c.downcast_ref::<ListBoxRow>() {
                    if let Some((cx, cy)) =
                        c.translate_coordinates(&list_overlay_begin, 0.0, 0.0)
                    {
                        rects.push((
                            row.clone(),
                            cx,
    ...
    let rects_update = row_rects.clone();
    ...
    drag_gesture.connect_drag_update(move |_, offset_x, offset_y| {
        ...
        if let Some((start_x, start_y)) = *start_pos_update.borrow() {
            ...
            for (row, cx, cy, cw, ch) in rects_update.borrow().iter() {
                let intersects =

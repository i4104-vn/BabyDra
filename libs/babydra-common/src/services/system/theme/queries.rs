...
pub fn get_current_appearance() -> CurrentAppearance {
    ...
    CurrentAppearance {
        gtk_theme: if gtk.is_empty() { "Adwaita".to_string() } else { gtk },
        icon_theme: if icon.is_empty() { "Adwaita".to_string() } else { icon },
        cursor_theme: if cursor.is_empty() { "Adwaita".to_string() } else { cursor },
        cursor_size: size,
    }
}

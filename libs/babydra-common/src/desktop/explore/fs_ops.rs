...
pub fn get_icon_name(path: &Path, is_dir: bool, mime: &str) -> String {
    ...
    if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
        match ext.to_lowercase().as_str() {
            ...
            "zip" | "tar" | "gz" | "bz2" | "xz" | "7z" | "rar" => return "package-x-generic".to_string(),
            "deb" | "rpm" => return "package-x-generic".to_string(),
            "exe" | "msi" | "appimage" | "sh" | "run" => return "application-x-executable".to_string(),
            _ => {}
        }
    }
...
pub async fn load_directory(path: PathBuf, show_hidden: bool) -> Result<Vec<FileEntry>, std::io::Error> {

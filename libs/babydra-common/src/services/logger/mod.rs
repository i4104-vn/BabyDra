...
impl BabyDraLogger {
    pub fn new(filename: &str) -> Self {
        ...
        if let Err(e) = std::fs::create_dir_all(&log_dir) {
            eprintln!("[LOGGER ERROR] Could not create log directory {:?}: {}", log_dir, e);
        }
        ...
        let file = OpenOptions::new()
            ...
            .open(&log_path)
            .map_err(|e| {
                eprintln!("[LOGGER ERROR] Could not open log file {:?}: {}", log_path, e);
                e
            })
            .ok();
...
pub fn init_logger(app_name: &str, filename: &str) -> PathBuf {
    ...
    logger.write_entry("INFO ", app_name, "==================================================");
    logger.write_entry("INFO ", app_name, &format!("{} session started", app_name));
    logger.write_entry("INFO ", app_name, &format!("Log file initialized at: {}", log_path.display()));
    logger.write_entry("INFO ", app_name, "==================================================");

    if let Err(e) = tracing::subscriber::set_global_default(logger) {
        eprintln!("[LOGGER ERROR] Failed to set global tracing subscriber for {}: {}", app_name, e);
    }

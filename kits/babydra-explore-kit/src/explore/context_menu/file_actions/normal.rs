use crate::explore::context_menu::{
    ...
    widgets::{create_footer_icon_button, create_menu_button},
    CLIPBOARD,
};
...
pub fn show_for_file_normal(
    ...
    let btn_open_new_window =
        create_menu_button(&t("explore.menu_open_new_window"), "external-link");
    ...
    let btn_decompress = if has_archive {
        Some(create_menu_button(
            &t("explore.menu_decompress"),
            "download",
        ))
    } else {
        None
    ...
    if is_single {
        ...
        btn_rename.connect_clicked(move |_| {
            pop_c.popdown();
            crate::explore::dialogs::show_rename_dialog(
                &rename_path,
                current_p.clone(),
                nav.clone(),
                Some(&parent_c),
    ...
    btn_trash.connect_clicked(move |_| {
        ...
        glib::spawn_future_local(async move {
            for path in paths_c {
                if let Err(err) = babydra_core::send_to_trash(path).await {
    ...
    btn_compress.connect_clicked(move |_| {
        pop_c.popdown();
        crate::explore::dialogs::show_compress_dialog(
        ...
        btn.connect_clicked(move |_| {
            ...
            for path in &target_paths_decompress {
                if is_archive_file(path) {
                    crate::explore::dialogs::perform_decompress_async(
                        path.clone(),
                        current_p.clone(),
                        nav.clone(),
                        Some(&parent_c),
    ...
    btn_properties.connect_clicked(move |_| {
        pop_c.popdown();
        crate::explore::dialogs::show_properties_dialog(

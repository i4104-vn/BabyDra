...
pub fn draw_summary_step(f: &mut Frame, app: &App, area: Rect) {
    ...
    let summary_lines = vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled("Installed Binaries:   ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            Span::styled("~/.local/bin & /var/lib/babydra/bin", Style::default().fg(Color::White)),
        ]),
        Line::from(vec![
            Span::styled("Compositor Config:    ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            Span::styled("~/.config/labwc (autostart, rc.xml, scripts)", Style::default().fg(Color::White)),
        ]),
        Line::from(vec![
            Span::styled("System Themes & Icons:", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            Span::styled("We10X icons, BabyDra GTK theme, Twilight cursors", Style::default().fg(Color::White)),
        ]),
        Line::from(""),
        Line::from(Span::styled("Next Steps & Launching:", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))),
        ...
        Line::from(""),
        Line::from(Span::styled("Press [Enter] or [q] to exit the installer.", Style::default().fg(Color::DarkGray))),
    ];

    let summary_widget = Paragraph::new(summary_lines)
        .block(
            Block::default()
                .title(" 8. Summary & Launch Instructions ")

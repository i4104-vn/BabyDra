...
pub fn build_header_bar_ui() -> HeaderBarWidgets {
    ...
    let btn_new_folder = babydra_ui_kit::components::create_icon_btn(
        "folder-new",
        &trans("explore.new_folder"),
        "toolbar-btn",
    );
    btn_new_folder.add_css_class("new-btn");
    btn_new_folder.set_tooltip_text(Some(&trans("explore.new_folder")));

    let btn_cut = babydra_ui_kit::components::create_icon_button(
        "cut",
        16,
        &["toolbar-btn"],
        Some(&trans("explore.cut")),
        || {},
    );

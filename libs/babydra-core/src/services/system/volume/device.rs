        ...
        if let Ok(val) = serde_json::from_slice::<serde_json::Value>(&out.stdout) {
            if let Some(arr) = val.as_array() {
                ...
                for card in card_devices {
                    ...
                    let card_desc = props
                        .and_then(|p| p.get("device.description").or(p.get("device.name")))
                        .and_then(|d| d.as_str())
                        .unwrap_or("Audio Card");

                    let params = card.get("info").and_then(|i| i.get("params"));
                    let mut active_profile_index = -1;
                    let mut active_profile_name = String::new();
                    if let Some(profile_arr) = params
                        .and_then(|p| p.get("Profile"))
                        .and_then(|p| p.as_array())
                    {
                        if !profile_arr.is_empty() {
                            active_profile_index = profile_arr[0]
                                .get("index")
                                .and_then(|idx| idx.as_i64())
                                .unwrap_or(-1);
                            active_profile_name = profile_arr[0]
                                .get("name")
                                .and_then(|n| n.as_str())
                                .unwrap_or("")
                                .to_string();
                    ...
                    if let Some(enum_profiles) = params
                        ...
                        .and_then(|ep| ep.as_array())
                    {
                        for prof in enum_profiles {
                            ...
                            let prof_available =
                                prof.get("available").and_then(|a| a.as_str()).unwrap_or("");
                            let prof_index =
                    ...
                    if let Some(enum_routes) = params
                        .and_then(|p| p.get("EnumRoute"))
                        .and_then(|er| er.as_array())
                    {
                        for route in enum_routes {
                            ...
                            if let Some(prof_ids) = route.get("profiles").and_then(|p| p.as_array())
                            {
                                ...
                                if let Some(pid_val) = best_pid {
                                    let is_default = pid_val == active_profile_index
                                        && active_routes.contains(r_name);
                    ...
                    if !is_source {
                        let mut outputs_grouped: std::collections::HashMap<
                            String,
                        ...
                        let mut inputs_grouped: std::collections::HashMap<
                            String,
                            Vec<&serde_json::Value>,
                        > = std::collections::HashMap::new();
                        for p in &valid_profiles {
                        ...
                        for (inp, profs) in inputs_grouped {
                            ...
                            for p in &profs {
                                let prof_index =
                                    p.get("index").and_then(|idx| idx.as_i64()).unwrap_or(-1);
                            ...
                            let desc =
                                if inp.contains("analog-stereo") || inp.contains("analog-mono") {
                                    "Microphone"
                                } else {
                                    best_p
                                        .get("description")
                ...
                for node in independent_nodes {
                    ...
                    let node_desc = props
                        .and_then(|p| p.get("node.description").or(p.get("node.name")))
                        .and_then(|d| d.as_str())

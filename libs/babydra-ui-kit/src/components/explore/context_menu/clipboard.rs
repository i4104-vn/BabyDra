use crate::components::explore::context_menu::CLIPBOARD;
        ...
        for (idx, src) in sources.into_iter().enumerate() {
            if let Some(filename) = src.file_name() {
                ...
                if is_cut {
                    ...
                    if let Err(e) = babydra_core::copy_path(src.clone(), dest.clone()).await {
        ...
        glib::spawn_future_local(async move {
            if op.is_cut {
                ...
                for dest in op.destinations {
                    if let Err(e) = babydra_core::delete_path(dest).await {

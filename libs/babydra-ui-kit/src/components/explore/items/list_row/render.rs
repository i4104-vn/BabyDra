...
pub fn build_list_row_ui(entry: &FileEntry) -> Box {
    ...
    // File size info
    let size_str = if matches!(entry.file_type, babydra_core::FileType::Directory) {
        "--".to_string()
    } else {
        crate::components::explore::format_size(entry.size)
    };
    ...
    let mod_str = if let Some(mtime) = entry.modified {
        crate::components::explore::format_date(mtime)
    } else {
        "--".to_string()
    };

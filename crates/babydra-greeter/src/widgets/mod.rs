...
pub fn create_logo_picture(size: i32) -> gtk4::Widget {
    let logo_bytes = include_bytes!("../../../../libs/babydra-common/src/services/logo.png");
    let stream = gtk4::gio::MemoryInputStream::from_bytes(&gtk4::glib::Bytes::from(logo_bytes));
    
    if let Ok(pixbuf) = gtk4::gdk_pixbuf::Pixbuf::from_stream_at_scale(&stream, size, size, true, gtk4::gio::Cancellable::NONE) {
...
pub fn create_avatar_picture(size: i32) -> gtk4::Widget {
    if let Some(bytes) = babydra_common::get_avatar_bytes() {
        if let Some(pixbuf) = babydra_common::crop_to_circle_pixbuf(&bytes, size) {
            let texture = gtk4::gdk::Texture::for_pixbuf(&pixbuf);
            let img = gtk4::Image::from_paintable(Some(&texture));
            img.set_pixel_size(size);

use gtk4::prelude::*;
use gtk4::{Align, Box, Button, Label, Orientation, Popover};

/// Creates a new `menu popover`.
...
pub fn create_menu_button(label: &str, icon: &str) -> Button {
    ...
    let img = babydra_ui_kit::ui::icon::get_icon(icon, 16);
    img.set_pixel_size(16);
    let lbl = Label::builder().label(label).halign(Align::Start).build();
...
/// Creates a new `footer icon button`.
pub fn create_footer_icon_button(icon: &str, tooltip: &str) -> Button {
    let img = babydra_ui_kit::ui::icon::get_icon(icon, 14);
...
/// Creates a new `footer container`.

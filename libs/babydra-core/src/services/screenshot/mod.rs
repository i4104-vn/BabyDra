...
fn apply_stroke_style(cr: &cairo::Context, color: (f64, f64, f64), width: f64) {
...
pub fn render_shape(cr: &cairo::Context, drawing: &Drawing) {
    match drawing {
        ...
        Drawing::Ellipse {
            ...
            apply_stroke_style(cr, *color, *width);
            // Sampled polyline keeps stroke width uniform under any transform
        ...
        Drawing::Arrow {
            ...
            // Shaft stops short of the arrowhead base
            cr.move_to(*x1, *y1);
...
///
/// `src` is the sampling rect in background-pixbuf coordinates; `dest` is the
/// rectangle to paint, in current context coordinates.
pub fn draw_pixelated_rect(
    ...
    // Downsample to a tiny mosaic then upscale with Nearest for crisp pixels
...
pub fn save_cropped_surface(state: &EditorState) -> Option<cairo::ImageSurface> {
    ...
    // Map canvas-space drawings into raw crop space
...
/// Geometry is intentionally left untouched when callers already applied the
/// coordinate transform via the cairo matrix.
fn scale_drawing(drawing: &Drawing, line_scale: f64) -> Drawing {
    match drawing {
        Drawing::Stroke {
            points,
            color,

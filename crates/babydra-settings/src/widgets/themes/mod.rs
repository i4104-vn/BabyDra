...
pub fn create_themes_widget() -> Widget {
    ...
    let widget = render::build(&gtk_themes, &icon_themes, &cursor_themes, &cursor_sizes);

    let gtk_themes_clone = gtk_themes.clone();
    let icon_themes_clone = icon_themes.clone();
    ...
    let icon_dropdown = widget.icon_theme_dropdown.clone();
    ...
    widget.apply_btn.connect_clicked(move |_| {
        ...
        let icon_idx = icon_dropdown.selected() as usize;
        ...
        let selected_icon = icon_themes_clone.get(icon_idx).cloned().unwrap_or_else(|| "Adwaita".to_string());
        ...
        let _ = babydra_common::services::system::theme::apply_appearance(&selected_gtk, &selected_icon, &selected_cursor, selected_size);
    });

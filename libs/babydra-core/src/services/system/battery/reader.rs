//! Sysfs battery and power supply reader.

use crate::models::system::battery::BatteryInfo;
use crate::services::system::battery::saver::apply_battery_saver;
use std::path::Path;
...
/// Returns the current `battery info`.
pub fn get_battery_info() -> Option<BatteryInfo> {
    let active_profile = Some(
        crate::services::system::power::profile::get_current_profile()
            .label()
            .to_string(),
    );
                ...
                if let Ok(kind) = std::fs::read_to_string(path.join("type")) {
                    if kind.trim() == "Battery" {
                        ...
                        let energy_full_design =
                            std::fs::read_to_string(path.join("energy_full_design"))
                                .or_else(|_| {
                                    std::fs::read_to_string(path.join("charge_full_design"))
                                })
                                .ok()
                                .and_then(|s| s.trim().parse::<f64>().ok());
                        ...
                        let capacity_wh = match (energy_now, energy_full) {
                            (Some(now), Some(full)) => Some(format!(
                                "{:.1} Wh / {:.1} Wh",
                                now / 1_000_000.0,
                                full / 1_000_000.0
                            )),
                        ...
                        let design_capacity = energy_full_design
                            .map(|design_val| format!("{:.1} Wh", design_val / 1_000_000.0));
                        ...
                        if is_charging {
                            if let Ok(secs_str) =
                                std::fs::read_to_string(path.join("time_to_full_now"))
                            {
                                if let Ok(secs) = secs_str.trim().parse::<u64>() {
                                    if secs > 0 && secs < 86400 {
                                        time_remaining =
                                            Some(format_duration((secs / 60) as u32, true));
                                    }
                                }
                            }
                            ...
                            if let Ok(secs_str) =
                                std::fs::read_to_string(path.join("time_to_empty_now"))
                            {
                                if let Ok(secs) = secs_str.trim().parse::<u64>() {
                                    if secs > 0 && secs < 86400 {
                                        time_remaining =
                                            Some(format_duration((secs / 60) as u32, false));
                                    }
                                }
                            }
                            if time_remaining.is_none() {

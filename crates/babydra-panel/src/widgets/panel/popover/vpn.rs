...
pub fn build_vpn_update_fn(vpn_icon: &gtk4::Image, vpn_popover: &gtk4::Popover) -> Rc<dyn Fn()> {
    ...
    Rc::new(move || {
        if let Some(active_vpn) = babydra_core::services::system::vpn::get_active_vpn_fast() {
            ...
            let mut rows = vec![
                TooltipRow::new("Status", "Active", None),
                TooltipRow::new("Name", &display_name, None),
                TooltipRow::new("Type", &proto_str, None),
            ];

            if !active_vpn.remote_server.is_empty() {
                rows.push(TooltipRow::new("Server", &active_vpn.remote_server, None));
            } else if !active_vpn.gateway.is_empty() {
                rows.push(TooltipRow::new("Gateway", &active_vpn.gateway, None));
            }

            if !active_vpn.ip_address.is_empty() {
                rows.push(TooltipRow::new("VPN IP", &active_vpn.ip_address, None));
            }

            if !active_vpn.username.is_empty() {
                rows.push(TooltipRow::new("User", &active_vpn.username, None));
            }

            if !active_vpn.dev_iface.is_empty() {
                rows.push(TooltipRow::new("Interface", &active_vpn.dev_iface, None));
            }

            let card = TooltipPopover::build_card("VPN Connection", &rows);

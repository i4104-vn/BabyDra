...
pub fn wire_events(widgets: &RecoveryWidgets) {
    ...
    // 3. Cancel Button
    let auth_overlay_cancel = widgets.auth_modal_overlay.clone();
    let pwd_entry_cancel = widgets.pwd_entry.clone();
    widgets.cancel_btn.connect_clicked(move |_| {
        pwd_entry_cancel.set_text("");
        auth_overlay_cancel.set_visible(false);
    });

    // 4. Confirm Reset Clicked -> Verify Sudo Password & Run Stream
    let auth_overlay_confirm = widgets.auth_modal_overlay.clone();
    let pwd_entry_confirm = widgets.pwd_entry.clone();
    let error_lbl_confirm = widgets.error_lbl.clone();
    let console_overlay_confirm = widgets.console_modal_overlay.clone();
    let status_badge_confirm = widgets.status_badge.clone();
    ...
    widgets.confirm_btn.connect_clicked(move |_| {
        ...
        // Hide auth modal and show modern console terminal modal
        error_lbl_confirm.set_visible(false);
        auth_overlay_confirm.set_visible(false);

        console_overlay_confirm.set_visible(true);
        status_badge_confirm.set_text(&trans("settings.recovery_badge_running"));
        status_badge_confirm.remove_css_class("terminal-badge-success");
        status_badge_confirm.remove_css_class("terminal-badge-failed");
        status_badge_confirm.add_css_class("terminal-badge-running");

        status_lbl_confirm.set_text(&trans("settings.recovery_step_init"));
        ...
        // Setup syntax highlighting tags if not already present
        let tag_table = buffer.tag_table();
        if tag_table.lookup("step_tag").is_none() {
            buffer.create_tag(
                Some("step_tag"),
                &[("foreground", &"#38bdf8"), ("weight", &700)],
            );
            buffer.create_tag(
                Some("success_tag"),
                &[("foreground", &"#4ade80"), ("weight", &700)],
            );
            buffer.create_tag(
                Some("error_tag"),
                &[("foreground", &"#f87171"), ("weight", &700)],
            );
            buffer.create_tag(Some("warn_tag"), &[("foreground", &"#fbbf24")]);
            buffer.create_tag(Some("dim_tag"), &[("foreground", &"#64748b")]);
        }
        ...
        let status_badge_poller = status_badge_confirm.clone();
        ...
        gtk4::glib::timeout_add_local(std::time::Duration::from_millis(50), move || {
            ...
            while let Ok(line) = rx.try_recv() {
                ...
                if line.starts_with('[') && line.contains("/6]") {
                    buf.insert_with_tags_by_name(
                        &mut end_iter,
                        &format!("{}\n", line),
                        &["step_tag"],
                    );
                } else if line.contains("Factory Reset Complete") || line.contains("Complete!") {
                    buf.insert_with_tags_by_name(
                        &mut end_iter,
                        &format!("{}\n", line),
                        &["success_tag"],
                ...
                } else if line.contains("Uninstalling")
                ...
                // Update progress fraction and status message
                if line.contains("[1/6]") {
                    progress_bar_poller.set_fraction(0.15);
                    status_lbl_poller.set_text(&line);
                    ...
                    status_badge_poller.set_text(&trans("settings.recovery_badge_done"));
                    status_badge_poller.remove_css_class("terminal-badge-running");
                    status_badge_poller.add_css_class("terminal-badge-success");
                    ...
                    status_badge_poller.set_text(&trans("settings.recovery_badge_failed"));
                    status_badge_poller.remove_css_class("terminal-badge-running");
                    status_badge_poller.add_css_class("terminal-badge-failed");

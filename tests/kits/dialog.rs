...
fn test_modern_dialog_builder_and_password_dialog_wiring() {
    ...
    assert!(
        first_child.is_some(),
        "Dialog container must contain the card!"
    );
    assert_eq!(
        first_child.as_ref(),
    ...
    assert_eq!(
        dialog.subtitle_label().map(|l| l.text()),
        Some("Please authenticate".into())
    );
    ...
    assert!(
        card_child.is_some(),
        "PasswordDialog container must contain the card!"
    );
    ...
    assert_eq!(
        auth_dlg.sub_lbl.as_ref().map(|l| l.text()),
        Some("Enter sudo password for removal:".into())
    );
    assert!(
        auth_dlg.container.is_visible(),
        "Container must be visible after show_for"
    );
    ...
    assert!(
        !auth_dlg.container.is_visible(),
        "Dialog should hide after submit"
    );
    ...
    assert!(
        !auth_dlg.container.is_visible(),
        "Dialog should hide after cancel"
    );
    assert_eq!(
        auth_dlg.password_entry.text(),
        "",
        "Password entry should be cleared after cancel"
    );
}

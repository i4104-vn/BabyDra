...
/// Extracts the underlying `GstPlay*` pointer from a raw GTK `GtkMediaFile` instance.
pub fn get_gst_play_from_media_file_raw(raw_ptr: *mut c_void) -> *mut c_void {
    if raw_ptr.is_null() {
...
pub fn set_media_file_speed_raw(raw_ptr: *mut c_void, speed: f64) {
    let play_ptr = get_gst_play_from_media_file_raw(raw_ptr);
    if !play_ptr.is_null() {
        unsafe {

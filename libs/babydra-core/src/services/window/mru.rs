    ...
    for (app_id, title) in running_windows {
        ...
        if matched_app.is_none() {
            for app in &desktop_apps {
                ...
                if app_id_lower.contains(&exec_name)
                    || exec_name.contains(&app_id_lower)
                    || title_lower.contains(&app.name.to_lowercase())
                    || app.name.to_lowercase().contains(&app_id_lower)
                {
                    matched_app = Some(app.clone());
                    break;
                }

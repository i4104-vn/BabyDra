...
pub fn save_cropped_surface(state: &EditorState) -> Option<cairo::ImageSurface> {
    ...
    // Map canvas coordinates to exact raw pixel coordinates
    ...
    let raw_x = ((state.crop_x * scale_x).round() as i32).clamp(0, bg_w - 1);
    let raw_y = ((state.crop_y * scale_y).round() as i32).clamp(0, bg_h - 1);
    let raw_w = ((state.crop_w * scale_x).round() as i32)
        .min(bg_w - raw_x)
        .max(1);
    let raw_h = ((state.crop_h * scale_y).round() as i32)
        .min(bg_h - raw_y)
        .max(1);

    // Extract exact 1:1 raw pixel buffer slice without any resampling
    ...
    // Paint raw pixels with Nearest filter to guarantee 0% interpolation blur
    ...
    // Overlay vector drawings on top of pixel-perfect base
    for drawing in &state.drawings {
        match drawing {
            Drawing::Blur { x, y, w, h } => {
                let bx = ((x - state.crop_x) * scale_x).round();
                let by = ((y - state.crop_y) * scale_y).round();
                let bw = (w * scale_x).round();
                let bh = (h * scale_y).round();
                draw_pixelated_rect(&cr, &sub_pb, bx, by, bw, bh);
            }
            Drawing::Stroke {
                points,
                color,
                ...
                cr.set_line_width(*width * ((scale_x + scale_y) / 2.0));
                cr.set_line_cap(cairo::LineCap::Round);
                cr.set_line_join(cairo::LineJoin::Round);
                let p0x = (points[0].0 - state.crop_x) * scale_x;
                let p0y = (points[0].1 - state.crop_y) * scale_y;
                cr.move_to(p0x, p0y);
                for p in &points[1..] {
                    let px = (p.0 - state.crop_x) * scale_x;
                    let py = (p.1 - state.crop_y) * scale_y;
                    cr.line_to(px, py);
                }
                cr.stroke().unwrap();
            }
            Drawing::Rect {
                ...
                let rx = ((x - state.crop_x) * scale_x).round();
                let ry = ((y - state.crop_y) * scale_y).round();
                let rw = (w * scale_x).round();
                let rh = (h * scale_y).round();
                cr.set_source_rgb(color.0, color.1, color.2);
                cr.set_line_width(*width * ((scale_x + scale_y) / 2.0));

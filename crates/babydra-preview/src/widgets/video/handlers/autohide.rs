...
/// Sets up autohide timer and hover-reveal controllers for controls_box and info_box.
pub fn setup_autohide_controls(
    ...
    motion_window.connect_motion(move |_, x, y| {
        ...
        // Bottom zone: bottom 90px (timeline position)
    ...
    glib::timeout_add_local(Duration::from_millis(100), move || {
        ...
        // If actively seeking, popover open, or viewing details: keep visible and refresh timers
        if is_seeking || is_popover {
        ...
        let controls_focused =
            in_bottom_check.get() || hover_c_check.get() || is_seeking || is_popover || is_details;
        if controls_focused {
            act_ctrl_check.set(Instant::now());
        ...
        let info_focused = in_tr_check.get() || hover_i_check.get() || is_details;
        if info_focused {
            act_info_check.set(Instant::now());

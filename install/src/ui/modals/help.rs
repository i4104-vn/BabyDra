...
pub fn draw_help_modal(f: &mut Frame, area: Rect) {
    ...
    let shortcuts = vec![
        ("1 - 8", "Jump directly to wizard step 1 through 8"),
        ("Tab / n", "Navigate to Next step"),
        ("BackTab / p", "Navigate to Previous step"),
        ("Up / Down / j / k", "Navigate items in the current active step"),
    ...
    let mut lines = vec![
        Line::from(Span::styled("BabyDra Step-by-Step TUI Installer - Shortcuts", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))),
        Line::from(""),
    ];
    ...
    lines.push(Line::from(Span::styled("Press [Esc] or [?] or [Enter] to close", Style::default().fg(Color::DarkGray))));

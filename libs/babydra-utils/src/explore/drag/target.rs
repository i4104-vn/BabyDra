            ...
            if !sources.is_empty() {
                ...
                glib::spawn_future_local(async move {
                    ...
                    for src in sources {
                        ...
                        if is_trash {
                            if let Ok(_) = babydra_common::send_to_trash(src.clone()).await {
                                actual_sources.push(src);
                            }
                        } else if let Some(filename) = src.file_name() {
                            let dest = dest_dir.join(filename);
                            if src != dest {
                                if let Ok(_) = babydra_common::move_path(src.clone(), dest.clone()).await {
                    ...
                    if !actual_sources.is_empty() {
                        ...
                        crate::explore::context_menu::clipboard::apply_cut_dimming_global(&[]);

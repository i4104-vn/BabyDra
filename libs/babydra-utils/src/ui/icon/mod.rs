...
pub fn get_icon(name: &str, size: i32) -> gtk4::Image {
    ...
    let svg = match (name, use_light_folder) {
        ...
        ("copy", false) | ("edit-copy-symbolic", false) => Some(DARK_COPY_SVG),
        ("copy", true) | ("edit-copy-symbolic", true) => Some(LIGHT_COPY_SVG),
        ("paste", false) => Some(DARK_PASTE_SVG),
        ("paste", true) => Some(LIGHT_PASTE_SVG),
        ("rename", false) | ("document-edit-symbolic", false) => Some(DARK_RENAME_SVG),
        ("rename", true) | ("document-edit-symbolic", true) => Some(LIGHT_RENAME_SVG),

...
use super::client::{
    get_wifi_device, owned_val_to_str, ConnectionSettingsProxyBlocking, SettingsProxyBlocking,
};
...
/// Returns the current `wifi config`.
pub fn get_wifi_config(ssid: &str) -> WifiConfig {
    ...
    if let Some(dev_path) = get_wifi_device(&conn) {
        if let Ok(builder) =
            super::client::DeviceProxyBlocking::builder(&conn).path(dev_path.clone())
        {
    ...
    if let Ok(settings) = SettingsProxyBlocking::new(&conn) {
        if let Ok(conns) = settings.list_connections() {
            for conn_path in conns {
                if let Some(c_settings) = ConnectionSettingsProxyBlocking::builder(&conn)
                    .path(conn_path)
                    .ok()
                    .and_then(|b| b.build().ok())
                {
                    if let Ok(details) = c_settings.get_settings() {
                        let is_target = details
                            .get("connection")
...
/// Sets `wifi config` to the given value.
pub fn set_wifi_config(ssid: &str, new_config: &WifiConfig) -> bool {
    let method = if new_config.method == "manual" {
        "manual"
    } else {
        "auto"
    };

...
pub fn get_vpn_connections() -> Vec<VpnConn> {
    ...
    if let Some(act_out) = run_cmd(&["nmcli", "-t", "-f", "NAME,TYPE,DEVICE", "connection", "show", "--active"]) {
    ...
    if let Some(stdout) = run_cmd(&["nmcli", "-t", "-f", "NAME,TYPE,UUID", "connection", "show"]) {
        for line in stdout.lines() {
            ...
            if parts.len() >= 2 && is_vpn_type(parts[1]) {
                ...
                if let Some(details_str) = run_cmd(&["nmcli", "-t", "-f", "IP4.ADDRESS,IP4.GATEWAY,vpn.user-name,vpn.data", "connection", "show", &name]) {
...
pub fn get_vpn_logs(name: &str, since: Option<&str>) -> String {
    let mut args = vec!["journalctl", "-u", "NetworkManager", "-n", "100", "--no-pager"];

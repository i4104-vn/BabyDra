...
pub fn save_vpn_connection(details: &VpnConnDetails) -> Result<(), String> {
    ...
    if exists {
        if orig_str != name_str {
            run_cmd_bool(&["nmcli", "connection", "modify", &orig_str, "connection.id", &name_str]);
        }
        if !un_str.is_empty() {
            run_cmd_bool(&["nmcli", "connection", "modify", &name_str, "vpn.user-name", &un_str]);
        }
        if !pw_str.is_empty() {
            run_cmd_bool(&["nmcli", "connection", "modify", &name_str, "vpn.secrets", &format!("password={}", pw_str)]);
            run_cmd_bool(&["nmcli", "connection", "modify", &name_str, "+vpn.data", "password-flags=0"]);
        }
        if !gw_str.is_empty() {
            run_cmd_bool(&["nmcli", "connection", "modify", &name_str, "+vpn.data", &format!("remote={}", gw_str)]);
        }
        if !ca_str.is_empty() {
            run_cmd_bool(&["nmcli", "connection", "modify", &name_str, "+vpn.data", &format!("ca={}", ca_str)]);
        }
        return Ok(());
    }

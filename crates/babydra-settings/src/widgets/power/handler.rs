...
pub fn wire_events(widget: &PowerWidget, auth_dialog: PasswordDialog) {
    ...
    widget
        .threshold_slider
        .set_value(init_conf.power.saver_threshold);
    ...
    let debounce_source_id: Rc<std::cell::Cell<Option<glib::SourceId>>> =
    ...
    std::thread::spawn(move || {
        let _ = babydra_core::services::system::battery::set_charge_limit(initial_limit);
    });
    ...
    let debounce_charge_id: Rc<std::cell::Cell<Option<glib::SourceId>>> =
        Rc::new(std::cell::Cell::new(None));
        ...
        if limit != last_saved_charge_limit.get() {
            ...
            let new_source_id =
                glib::timeout_add_local(std::time::Duration::from_millis(350), move || {
                    ...
                    let res = babydra_core::services::system::battery::set_charge_limit(limit);
                    match res {
                        Ok(()) => {
                            let mut conf = load_babydra_config();

...
pub(crate) fn create_sidebar_icon(id: &str, default_icon: &str) -> gtk4::Widget {
    match id {
        "wifi" => babydra_ui_kit::components::create_wifi_icon(18),
    ...
    if let Some(child) = btn.child() {
        if let Ok(hbox) = child.downcast::<gtk4::Box>() {
            ...
            while let Some(w) = widget {
                ...
                if is_first {
                    hbox.remove(&w);
                    let new_icon = create_sidebar_icon(id, default_icon);
...
pub(crate) fn refresh_sidebar(nav_buttons: &Rc<RefCell<Vec<NavButtonEntry>>>) {
    for (id, btn, key, icon) in nav_buttons.borrow().iter() {
        update_sidebar_icon(id, btn, &babydra_core::i18n::trans(key), icon);
    }
}

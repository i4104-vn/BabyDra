...
fn launch_settings_app() {
    std::thread::spawn(|| {
        if std::process::Command::new("babydra-settings").spawn().is_ok() {
            return;
        }
        if let Ok(exe_path) = std::env::current_exe() {
            if let Some(parent) = exe_path.parent() {
                let local_settings = parent.join("babydra-settings");
                if local_settings.exists() && std::process::Command::new(&local_settings).spawn().is_ok() {
                    return;
                }
            }
        }
...
pub fn create_header_row(
    _on_popover_toggled: Option<Rc<dyn Fn(bool) + 'static>>,
) -> gtk4::Box {

...
[ -f target/release/babydra-notepad ] && cp target/release/babydra-notepad "$LOCAL_BIN/babydra-notepad"
...
if [ -d "$REPO_DIR/desktops" ]; then
    cp "$REPO_DIR/desktops/"*.desktop "$HOME/.local/share/applications/"
